...
pub struct HostFnWeights<T: Config> {
	...
	pub restore_to_per_code_byte: Weight,
...
impl<T: Config> Default for HostFnWeights<T> {
	fn default() -> Self {
		Self {
			...
			restore_to_per_code_byte: cost_byte_args!(seal_restore_to_per_code_kb_delta, 1, 0),
			restore_to_per_delta: cost_batched_args!(seal_restore_to_per_code_kb_delta, 0, 1),
